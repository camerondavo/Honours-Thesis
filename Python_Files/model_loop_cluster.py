...
#
def fit_models(N_bar_true, D_true, Q_true, pathname):
...
#perform model 1 fitting (dipole)
    ...
    dipole_sampler = ultranest.ReactiveNestedSampler(dipole_param_names, vectorised_dipole_likelihood, vectorised_dipole_prior,
                    log_dir=pathname+f'dipole_model/dipole_Nbar_{N_bar_true}_D_{D_true}_Q_{Q_true}', vectorized=True,
                                                    resume='resume-similar')
...
#perform model 2 fitting (quadrupole)
    ...
    quad_sampler = ultranest.ReactiveNestedSampler(quad_param_names, vectorised_quadrupole_likelihood, vectorised_quadrupole_prior, 
        log_dir=pathname+f'quadrupole_model/quadrupole_Nbar_{N_bar_true}_D_{D_true}_Q_{Q_true}', vectorized=True,
                                                    resume='resume-similar')
...
for N_bar_true, D_true, Q_true in param_combinations:
    ...
    fit_models(N_bar_true, D_true, D_true, pathname)
    del data, m, metadata
    gc.collect()
